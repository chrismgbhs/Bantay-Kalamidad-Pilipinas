    ...
    public class admindashboard_rescue_Assignments_viewModel : ObservableObject, IAdminRescueModuleViewModel
    {
        ...
        private async Task PrepareAddModeAsync()
        {
            try
            {
                ...
                {
                    string firstId = await DatabaseManager.GenerateAssignmentIdAsync();
                    _pendingIdSequence = int.Parse(firstId.Substring(2)); // skip "OP"
                }
                else
                {
                    _pendingIdSequence++;
                ...
                string newId = "OP" + _pendingIdSequence.ToString("D4");
        ...
        private async Task LoadAssignmentsAsync()
        {
            try
            {
                ...
                Application.Current.Dispatcher.Invoke(() => { Assignments = loaded; _pendingIdSequence = -1; });
            }
            catch (SqlException)
            {
